// LCSI AI Chatbot - Core Application Logic (Client-Side RAG & Security)
...
// Action: Handle Chat Inquiry (RAG Pipeline & Guardrail)
chatForm.addEventListener('submit', async (e) => {
  ...
  // 4. Secure API Call to Gemini (No backend, client-direct)
  const apiKey = currentSession.apiKey;
  if (!apiKey) {
    toggleTyping(false);
    appendMessage('assistant', "안내: 실시간 AI 해석 답변을 받으려면 우측 상단 톱니바퀴 설정 아이콘을 클릭하여 개인 구글 'Gemini API Key'를 등록해 주셔야 합니다.\n\n(API Key 없이도 위기 감지 및 로컬 전처리 분석 기능은 현재 정상 동작 중입니다.)");
    return;
  }
  ...
  try {
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          temperature: 0.2, // Low temperature to secure hallucination control
          maxOutputTokens: 800
        }
      })
    });
    ...
    if (aiText) {
      ...
      appendMessage('assistant', "죄상합니다, 답변을 가져오는 과정에서 오류가 발생했습니다.");
    }
